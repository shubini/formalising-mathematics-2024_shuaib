...
lemma inter_non_empty_if_sum_of_dim_greater_than_dim_of_space {A B : Subspace k V} (n m1 m2 : ℕ)
    (hV : finrank k V = n) (hA : finrank k A = m1) (hB : finrank k B = m2)
    (hS: m1 + m2 > n) : A ⊓ B ≠ ⊥ := by
...
theorem exists_a_subspace_directsum {k : Type} [Field k] {V : Type} [AddCommGroup V] [Module k V] [FiniteDimensional k V]
  ...
  · use (⊥: Submodule k V) -- use the 0 dimensional subspace
    have h1: W  = (⊤: Submodule k V) := by
      ext w
      constructor
      · exact fun a => trivial
      · intro hw
        have h2: finrank k (⊤: Submodule k V) = n := by exact finrank_top k V
        have h3: W ≃ₗ[k] (⊤: Submodule k V) := by
          rw [←h2] at h
          change finrank k ↥W = finrank k ↥(⊤: Submodule k V) at h
          exact fvs_are_iso_if_they_have_the_same_dim h
        have B: Basis (Fin n) k (⊤: Submodule k V) := by
          exact finBasisOfFinrankEq k (⊤: Submodule k V) h2
        sorry
  ...
  · -- for this case, I think the way it's generally done is by taking a basis of W, adding vectors to it to a basis of V and then
    --taking U to be the space generated by those vectors added. I don't know how to do this right now.
    sorry
  done
